...
class BlogImpressions_Api {
	...
	function __construct($appid = '', $key = '', $secret = '', $path = '') {
		...
		$this->secret = $secret;
		$this->path = $path;
		$this->api_host = parse_url("https://zvktzv.blogimpressions.com", PHP_URL_HOST);
	...
	private function api($method, $params) {
		...
		if (is_admin()) {
			...
			if ($this->registered) {
				...
				//return '<span style="color: red;">'.__('Your blogimpressions plugin is not setup yet', 'blogimpressions').'</span>';
				return '';
			}
		}
	}
	...
	private function call($params) {
		...
		if (!$fs) {
			if (isset($params['check'])) {
				return '<div class="error blogimpressions" style="text-align: left; ">' . $this->call_error($errstr, $errno) . '</div>';
			}
	...
	private function call_error($errstr, $errno) {
		return "<p style='color:red;'>" . BLOGIMPRESSIONS_CONNECT_ERROR . "</p><a style='text-decoration:underline;' href='javascript:void(0)' onclick='javascript:jQuery(\"#blogimpressions-error-details\").toggle();'>Details</a><span id='blogimpressions-error-details' style='display: none;'><br>$errstr ($errno)</span>";
	}
